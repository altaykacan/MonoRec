...
class DeepScenarioTUMMultiDataset(Dataset):

    def __init__(self, dataset_dirs, **kwargs):
        if isinstance(dataset_dirs, list):
            self.datasets = [DeepScenarioTUMDataset(dataset_dir, **kwargs) for dataset_dir in dataset_dirs]
        else:
            self.datasets = [DeepScenarioTUMDataset(dataset_dirs, **kwargs)]
...
class DeepScenarioTUMDataset(Dataset):
    ...
    def __init__(self, dataset_dir, frame_count=2, target_image_size=(480, 640), max_length=None, dilation=1, only_keyframes=False, color_augmentation=True, scale_factor=1):
        ...
        # Ignoring photometric calibration
